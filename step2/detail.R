...
for (i in 1:100){
  ...
  if(is.element(snp[i], f80303$snp)){
    ...
    maineff.80303[i] = f80303$maincoef[index]
    mainsd.80303[i] = f80303$mainsd[index]
    tempbeta <- tempbeta + maineff.80303[i]/(mainsd.80303[i]^2)
    tempsd <- tempsd + 1.0/(mainsd.80303[i]^2)
  }
  if(is.element(snp[i], f80405$snp)){
    ...
    maineff.80405[i] = f80405$maincoef[index]
    mainsd.80405[i] = f80405$mainsd[index]
    tempbeta <- tempbeta + maineff.80405[i]/(mainsd.80405[i]^2)
    tempsd <- tempsd + 1.0/(mainsd.80405[i]^2)
  }
  if(is.element(snp[i], f40503$snp)){
    ...
    maineff.40503[i] = f40503$maincoef[index]
    mainsd.40503[i] = f40503$mainsd[index]
    tempbeta <- tempbeta + maineff.40503[i]/(mainsd.40503[i]^2)
    tempsd <- tempsd + 1.0/(mainsd.40503[i]^2)
  }
  main[i] <- tempbeta/tempsd
  mainsd[i] <- sqrt(1.0/tempsd)
}

detail <- data.frame(top100, main, mainsd, eff.80303, sd.80303, maf.80303, 
...
png(paste(wd, "qq80303_alt.png", sep = ""), width=1000, height=1000, pointsize=22)
qq(f80303$pvalue, xlim=c(0,8), ylim=c(0,8))
dev.off()

png(paste(wd, "qq80405_alt.png", sep = ""), width=1000, height=1000, pointsize=22)
qq(f80405$pvalue, xlim=c(0,8), ylim=c(0,8))
dev.off()
