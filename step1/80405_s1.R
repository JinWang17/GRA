...
mc1 = check.marker(df1, callrate = 0.95, extr.call = 0.95, 
                  p.level = 1e-08, het.fdr = 0, ibs.exclude = "lower")
df1 = df1[mc1$idok, mc1$snpok]
mc2 = check.marker(df2, callrate = 0.95, extr.call = 0.95, 
                  p.level = 1e-08, het.fdr = 0, ibs.exclude = "lower")
...
mc = check.marker(df, callrate = 0.95, extr.call = 0.95, p.level = 1e-08, 
                  het.fdr = 0, ibs.exclude = "lower")
...
for(i in 1:nsnps){
  ...
  fit <- summary(coxph(Surv(time, status) ~ snps[,i]*phdata(df1)$Bev + snps[,i]
                       + phdata(df1)$Bev + phdata(df1)$Cetux   
...
for(i in 1:nsnps){
  ...
  fit <- summary(coxph(Surv(time, status) ~ snps[,i]*phdata(df2)$Bev + snps[,i]
                       + phdata(df2)$Bev + phdata(df2)$Cetux   
