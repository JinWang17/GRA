...
load("/lustre/scr/j/i/jinjin/GRA/meta_analysis/80303.Rdata")
...
for(i in 1:nsnps){
  ...
  int <- snps[,i]*arm
  fit <- summary(coxph(Surv(time, status) ~ snps[,i] + int + arm
...
write.table(result, "/lustre/scr/j/i/jinjin/GRA/meta_analysis/result_main/80303.txt", 
            sep="\t")
