...
for(i in 1:nsnps){
  ...
  fit <- summary(coxph(Surv(time, status) ~ snps[,i] + phdata(df)$Bev
                       + loc1 + loc2 + loc3 + loc4 
...
write.table(result, "/lustre/scr/j/i/jinjin/GRA/meta_analysis/result_main2/80405.txt", 
            sep="\t")
...
for(i in 1:nsnps){
  ...
  fit <- summary(coxph(Surv(time, status) ~ snps[,i] + phdata(df1)$Bev
                       + loc1 + loc2 + loc3 + loc4 
...
write.table(result, "/lustre/scr/j/i/jinjin/GRA/meta_analysis/result_main2/80405_1.txt", 
            sep="\t")
...
for(i in 1:nsnps){
  ...
  fit <- summary(coxph(Surv(time, status) ~ snps[,i] + phdata(df2)$Bev
                       + loc1 + loc2 + loc3 + loc4 
...
write.table(result, "/lustre/scr/j/i/jinjin/GRA/meta_analysis/result_main2/80405_2.txt", 
            sep="\t")
